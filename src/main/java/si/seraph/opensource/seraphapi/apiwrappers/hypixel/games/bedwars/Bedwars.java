...
public final class Bedwars extends BedwarsUtils implements IHypixelGame {
	...
	private final String queuePrefix = MessageFormat.format("{0}*{1}", ChatColour.LIGHT_PURPLE, ChatColour.RESET);
	private final String separator = ChatColour.GOLD + "-" + ChatColour.RESET;
	private final String joinPrefix = MessageFormat.format("{0}+{1}", ChatColour.GREEN, ChatColour.RESET);
	...
	private JsonObject getAchievementObject() {
	...
	public String getFormattedQueueStats() {
		if (!isNicked) {
			if (!notReal) {
				...
				return MessageFormat.format("{0}{1} is not a real player!", ChatColour.RED, getPlayerName());
			}
		} else {
			return MessageFormat.format("{0}{1} is nicked!", ChatColour.RED, getPlayerName());
		}
	}
